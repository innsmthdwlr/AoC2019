...
def adjacent_are_the_same(password_candidate):
    password_candidate_str = str(password_candidate)
    if len(set(password_candidate_str)) <= 5 and password_candidate_str == ''.join(sorted(password_candidate_str)):
        return True
    return False

def adjacent_are_the_same_p2(password_candidate):
    password_candidate_str = str(password_candidate)
    if 2 in Counter(password_candidate_str).values() and password_candidate_str == ''.join(sorted(password_candidate_str)):
        return True
    return False
